...
(function() {
    ...
    waitForJQuery(function($) {
        const existingRuntime = window.__pkmTavernInjectState;
        if (existingRuntime && typeof existingRuntime.teardown === 'function') {
            console.log('[PKM] 检测到旧实例，准备先卸载再重载...');
            try {
                existingRuntime.teardown('reload');
            } catch (e) {
                console.warn('[PKM] 卸载旧实例失败:', e);
            }
        }
        ...
        const runtimeState = {
            ...
            topWindowMessageHandler: null,
            pagehideHandler: null,
            keydownHandler: null,
            onEraWriteDone: null,
            onGenerationEnded: null,
            onChatChanged: null,
            onRefreshPokemonSpawns: null,
            topWindowRef: null
        };
        window.__pkmTavernInjectState = runtimeState;
        ...
        // ========== ERA 变量获取 ==========
        const ERA_VARS_CACHE_TTL_MS = 250;
        const CHAT_SCAN_CACHE_TTL_MS = 1500;
        const locationContextState = {
            hydratedWorldKey: null,
            lastSignature: null,
            promptContent: null,
            npcScanKey: null,
            npcTriggerSection: '',
        ...
        function setEraVarsCache(value) {
            eraVarsCache.value = value;
            eraVarsCache.fetchedAt = Date.now();
            return value;
        }

        function invalidateEraVarsCache() {
            eraVarsCache.value = null;
            eraVarsCache.fetchedAt = 0;
            eraVarsCache.promise = null;
        }

        async function getEraVars(options = {}) {
            const force = options.force === true;
            const maxAgeMs = options.maxAgeMs ?? ERA_VARS_CACHE_TTL_MS;
            ...
            eraVarsCache.promise = null;
            return result ? setEraVarsCache(result) : result;
        }
        ...
        function buildWeatherSignature(currentWeatherConfig, nearbyWeathers) {
            const currentSig = currentWeatherConfig
                ? `${currentWeatherConfig.weather || 'clear'}:${(currentWeatherConfig.suppression?.suppressed || []).join('/')}`
                : 'none';
            const nearbySig = nearbyWeathers
                .map(({ dir, weather, suppression }) => `${dir}:${weather}:${(suppression?.suppressed || []).join('/')}`)
                .sort()
                .join('|');
            return `${currentSig}__${nearbySig}`;
        }

        function buildPokemonSignature(currentPokemon) {
            return (currentPokemon || [])
                .map(poke => `${poke.id || 'unknown'}:${poke.level || ''}:${poke.rarity || ''}`)
                .join('|');
        }
        ...
        function getRecentMessagesForNpcScan(historyDepth = 10) {
            ...
            return getChatMessages(`${startMessageId}-${lastMessageId}`) || [];
        }

        function getNpcTriggerSection(force = false) {
            const historyDepth = 10;
            const messages = getRecentMessagesForNpcScan(historyDepth);
            if (!messages || messages.length === 0) {
                return '';
            }

            const scanKey = messages.map(m => {
            ...
            if (!force &&
                locationContextState.npcScanKey === scanKey &&
                (Date.now() - locationContextState.npcScanAt) < CHAT_SCAN_CACHE_TTL_MS) {
                return locationContextState.npcTriggerSection;
            }
            ...
            locationContextState.npcScanKey = scanKey;
            locationContextState.npcTriggerSection = npcTriggerSection;
            locationContextState.npcScanAt = Date.now();
            return npcTriggerSection;
        }
        
        // ========== 位置上下文注入函数 ==========
        async function injectLocationContext(options = {}) {
            const force = options.force === true;

            if (!force && locationContextState.inFlightPromise) {
                return locationContextState.inFlightPromise;
            }
            ...
            try {
                ...
                const eraVars = await getEraVars({ force: options.forceEraFetch === true });
                if (!eraVars) {
                ...
                const hydrationWorldKey = `${location.x},${location.y}|${currentRegion}|${currentDay}`;

                if (force || locationContextState.hydratedWorldKey !== hydrationWorldKey) {
                    ...
                    if (newSpawns && Object.keys(newSpawns).length > 0) {
                        ...
                        if (!eraVars.world_state.pokemon_spawns) eraVars.world_state.pokemon_spawns = {};
                        Object.assign(eraVars.world_state.pokemon_spawns, newSpawns);
                    }
                    ...
                    if (newWeatherGrid && Object.keys(newWeatherGrid).length > 0) {
                        ...
                        if (!eraVars.world_state) eraVars.world_state = {};
                        if (!eraVars.world_state.weather_grid) eraVars.world_state.weather_grid = {};
                        Object.assign(eraVars.world_state.weather_grid, newWeatherGrid);
                    }

                    locationContextState.hydratedWorldKey = hydrationWorldKey;
                    setEraVarsCache(eraVars);
                } else {
                    console.log('[PKM] [CTX] 位置与日期未变化，跳过周边 spawn/weather 补全');
                }
                ...
                const weatherSignature = buildWeatherSignature(currentWeatherConfig, nearbyWeathers);
                const pokemonSignature = buildPokemonSignature(currentPokemon);
                
                const npcTriggerSection = getNpcTriggerSection(force);
                const contextSignature = JSON.stringify({
                    key: hydrationWorldKey,
                    weather: weatherSignature,
                    pokemon: pokemonSignature,
                    npc: npcTriggerSection
                });

                if (!force && locationContextState.lastSignature === contextSignature && locationContextState.promptContent) {
                    console.log('[PKM] [CTX] 上下文未变化，跳过重复注入');
                    return;
                }
...
${contextText}
                ...
                if (typeof injectPrompts === 'function') {
                    ...
                    locationContextState.lastSignature = contextSignature;
                    locationContextState.promptContent = promptContent;
            ...
            locationContextState.inFlightPromise = task;
            try {
                return await task;
            } finally {
                if (locationContextState.inFlightPromise === task) {
                    locationContextState.inFlightPromise = null;
                }
            }
        }
        ...
        runtimeState.keydownHandler = onDocumentKeydown;
        ...
        async function refreshDashboard() {
            ...
            refreshDashboardTimer = setTimeout(async () => {
                const canPostToVisibleIframe =
                    visibleIframeLoaded &&
                    overlay.css('display') !== 'none' &&
                    iframe[0] &&
                    iframe[0].contentWindow;
                if (!canPostToVisibleIframe) {
        ...
        function onChatChanged() {
            ...
            invalidateEraVarsCache();
            locationContextState.lastSignature = null;
            locationContextState.promptContent = null;
            locationContextState.npcScanKey = null;
            locationContextState.npcTriggerSection = '';
            injectLocationContext({ force: true }); // 切换对话时也刷新位置注入
        }

        async function onRefreshPokemonSpawns(detail) {
                ...
                try {
                    ...
                    invalidateEraVarsCache();
                    locationContextState.hydratedWorldKey = null;
                    locationContextState.lastSignature = null;
        ...
        runtimeState.onEraWriteDone = onEraWriteDone;
        runtimeState.onGenerationEnded = onGenerationEnded;
        runtimeState.onChatChanged = onChatChanged;
        runtimeState.onRefreshPokemonSpawns = onRefreshPokemonSpawns;
        ...
        if (typeof eventOn !== 'undefined') {
            ...
            eventOn('pkm:refreshPokemonSpawns', onRefreshPokemonSpawns);
        }
        ...
        function unloadPkmUI() {
            ...
            if (typeof eventRemoveListener !== 'undefined') {
                try {
                    if (runtimeState.onEraWriteDone) eventRemoveListener('era:writeDone', runtimeState.onEraWriteDone);
                    if (runtimeState.onGenerationEnded) eventRemoveListener('generation_ended', runtimeState.onGenerationEnded);
                    if (runtimeState.onChatChanged) eventRemoveListener('chat_changed', runtimeState.onChatChanged);
                    if (runtimeState.onRefreshPokemonSpawns) eventRemoveListener('pkm:refreshPokemonSpawns', runtimeState.onRefreshPokemonSpawns);
                } catch (e) {
                    // 忽略清除失败
                }
            }
            ...
            locationContextState.npcScanKey = null;
            locationContextState.npcTriggerSection = '';
            locationContextState.npcScanAt = 0;
            locationContextState.inFlightPromise = null;
            isProcessingEraEvent = false;
            transferBufferLock = false;
            
            // 清理全局变量
            delete window.pkmDashboard;
            if (window.pkmSetLeader === handleLeaderToggle) delete window.pkmSetLeader;
            if (window.pkmUpdateSettings === handleSettingsToggle) delete window.pkmUpdateSettings;
            if (window.__pkmTavernInjectState === runtimeState) delete window.__pkmTavernInjectState;
            
            // 移除 pagehide 监听器
            window.removeEventListener('pagehide', unloadPkmUI);
            if (runtimeState.keydownHandler) {
                $(document).off('keydown', runtimeState.keydownHandler);
            }
            if (runtimeState.windowMessageHandler) {
                window.removeEventListener('message', runtimeState.windowMessageHandler);
            }
            if (runtimeState.topWindowRef && runtimeState.topWindowMessageHandler) {
                runtimeState.topWindowRef.removeEventListener('message', runtimeState.topWindowMessageHandler);
            }
            if (runtimeState.topWindowRef && runtimeState.topWindowRef.pkmSetLeader === handleLeaderToggle) {
                delete runtimeState.topWindowRef.pkmSetLeader;
            }
            if (runtimeState.topWindowRef && runtimeState.topWindowRef.pkmUpdateSettings === handleSettingsToggle) {
                delete runtimeState.topWindowRef.pkmUpdateSettings;
            }
            
            console.log('[PKM] UI 脚本卸载完成');
        }

        runtimeState.teardown = unloadPkmUI;
        runtimeState.pagehideHandler = unloadPkmUI;
        ...
        runtimeState.windowMessageHandler = onWindowMessage;
        ...
        try {
            ...
            if (topWin && topWin !== window) {
                ...
                runtimeState.topWindowMessageHandler = onTopWindowMessage;
                topWin.addEventListener('message', onTopWindowMessage);
