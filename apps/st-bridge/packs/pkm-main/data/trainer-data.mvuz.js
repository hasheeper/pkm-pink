/**
 * PKM Main fixed trainer data.
 * Generated from apps/battle-sim/data/trainer-data.js and wrapped for pack-local loading.
 */
(function () {
  'use strict';
...
if (typeof window !== 'undefined') {
  ...
  const TRAINER_ALIASES = {
    hexmaniac: 'hex', hexgirl: 'hex', occultmaniac: 'hex',
    ionostreamer: 'iono', nanjamo: 'iono',
    championcynthia: 'cynthia', shirona: 'cynthia',
    gymleadernessa: 'nessa', gymleaderbea: 'bea', gymleadererika: 'erika', gymleadermarnie: 'marnie',
    professorsonia: 'sonia', captainmallow: 'mallow', captainlana: 'lana',
    aetherpresidentlusamine: 'lusamine', presidentlusamine: 'lusamine',
    hikari: 'dawn', mei: 'rosa', serenakalos: 'serena', yuuri: 'gloria', gloriagalar: 'gloria'
  };
  ...
  function getTrainerDatabase() {
    return ROOT.TRAINER_GLOBALS || {};
  }

  function getNpcDataApi() {
    return RUNTIME.data && RUNTIME.data.npc ? RUNTIME.data.npc : null;
  }

  function resolveTrainerKey(name) {
    const db = getTrainerDatabase();
    const normalized = normalizeTrainerKey(name);
    if (!normalized) return null;
    if (db[normalized]) return normalized;
    if (TRAINER_ALIASES[normalized] && db[TRAINER_ALIASES[normalized]]) return TRAINER_ALIASES[normalized];
    const npcKey = getNpcDataApi()?.normalizeNpcKey?.(name);
    if (npcKey && db[npcKey]) return npcKey;
    return Object.keys(db).find((key) => normalized.includes(key) || key.includes(normalized)) || null;
  }

  function getTrainerDisplayName(key, fallback) {
    const addon = getNpcDataApi()?.getNpcAddon?.(key);
    return addon?.name_en || fallback || key.charAt(0).toUpperCase() + key.slice(1);
  }

  function extractParty(tierData) {
    if (Array.isArray(tierData)) return tierData;
    if (tierData && Array.isArray(tierData.party)) return tierData.party;
    return null;
  }

  function extractUnlocks(tierData) {
    return tierData && !Array.isArray(tierData) && tierData.unlocks ? tierData.unlocks : null;
  }

  function extractTrainerProficiency(tierData) {
    return tierData && !Array.isArray(tierData) && typeof tierData.trainerProficiency === 'number'
      ? tierData.trainerProficiency
      : 0;
  }

  function selectTier(trainerData, requestedTier, key) {
    const validTier = Math.max(1, Math.min(4, Math.round(Number(requestedTier) || 2)));
    if (extractParty(trainerData && trainerData[validTier]) && extractParty(trainerData[validTier]).length) return validTier;
    const available = Object.keys(trainerData || {}).filter((item) => /^\d+$/.test(item)).map(Number).sort((a, b) => a - b);
    if (!available.length) return validTier;
    if (validTier === 3 && !available.includes(3) && available.includes(4)) return 4;
    if (validTier === 1 && key === 'cynthia' && !available.includes(1) && available.includes(2)) return 2;
    return available.reduce((best, current) => (
      Math.abs(current - validTier) < Math.abs(best - validTier) ? current : best
    ), available[0]);
  }

  function lookupTrainer(name, requestedTier = 2) {
    const db = getTrainerDatabase();
    const key = resolveTrainerKey(name);
    if (!key || !db[key]) return null;
    const trainerData = db[key];
    const tier = selectTier(trainerData, requestedTier, key);
    const tierData = trainerData[tier];
    const party = extractParty(tierData);
    if (!party || !party.length) return null;
    const difficulty = ({ 1: 'easy', 2: 'hard', 3: 'expert', 4: 'expert' })[tier] || 'normal';
    return {
      key,
      requestedName: String(name || ''),
      name: getTrainerDisplayName(key),
      tier,
      party: clone(party, []),
      unlocks: clone(extractUnlocks(tierData), null),
      trainerProficiency: extractTrainerProficiency(tierData),
      difficulty
    };
  }

  RUNTIME.data.trainers = getTrainerDatabase();
  RUNTIME.data.trainer = {
