...
const battleState = {
    gimmickType: 'dyna', // 'mega' | 'zmove' | 'dyna' | 'none' | 'tera'
    canGimmick: true,
    canEvo: true,
    evoType: 'bio', // 'bio' | 'bond' | 'none'
    canStyle: true,
    canCommand: true,
    currentStyle: null,
};
...
const MENU_LAYERS = {
    ...
    style: {
        ...
        left: { label: '<span class="c1">凝</span><span class="c2">神</span>', sub: '', theme: 'theme-style-pure-focus', icon: null, action: () => selectStyle(null) },
...
function renderWheel(type) {
    ...
    QUADRANTS.forEach((pos) => {
        ...
        if (subEl) subEl.innerText = isLocked ? '不可用' : config.sub;
        ...
        if (type === 'style') {
            const activeStyleMatch =
                (pos === 'top' && battleState.currentStyle === 'agile') ||
                (pos === 'bottom' && battleState.currentStyle === 'strong') ||
                (pos === 'left' && battleState.currentStyle === null);
            if (activeStyleMatch) btn.classList.add('active-selection');
        }
...
function activateGimmick() {
    console.log('Mega Evolution / Z-Move Triggered!');
    setMenuLevel('hidden');
    SmartBubbleManager.refresh();
}

function activateEvolution() {
    console.log('Combat Evolution Triggered!');
    setMenuLevel('hidden');
    SmartBubbleManager.refresh();
}

function selectStyle(style) {
    battleState.currentStyle = style;
    console.log(`Style Switched to: ${style || 'Normal'}`);
    setMenuLevel('hidden');
    SmartBubbleManager.refresh();
}
...
const SmartBubbleManager = {
    ...
    scanBattleState() {
        ...
        if (battleState.currentStyle) {
            this.prepareLockedState('style');
            return;
        }
    ...
    prepareLockedState(category) {
        if (category === 'style') {
            if (battleState.currentStyle === 'agile') {
                this.lockedState = { icon: null, text: '迅', label: 'AGILE', themeColor: '#00d2d3' };
            } else if (battleState.currentStyle === 'strong') {
                this.lockedState = { icon: null, text: '刚', label: 'STRONG', themeColor: '#ff4757' };
            } else {
                this.lockedState = { icon: null, text: '定', label: 'FOCUS', themeColor: '#2f9e44' };
            }
        }
